...
func (r *VideoRepo) UpdateMeta(ctx context.Context, id uuid.UUID, sha string, dur int, vcodec string, acodec string, w int, h int) error {
	_, err := r.pool.Exec(ctx, `
...
func (r *VideoRepo) UpdateStatus(ctx context.Context, id uuid.UUID, status models.VideoStatus) error {
	_, err := r.pool.Exec(ctx, `UPDATE videos SET status=$2, updated_at=now() WHERE id=$1`, id, status)
	return err
}

func (r *VideoRepo) UpdatePublish(ctx context.Context, id uuid.UUID, qualities []string, manifest string, thumbs any) error {
...
func (r *VideoRepo) Get(ctx context.Context, id uuid.UUID) (*models.Video, error) {
