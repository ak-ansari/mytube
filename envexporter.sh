...
export MINIO_REDIS_QUEUE_NAME=$(yq -r '.S3.MINIO_REDIS_QUEUE_NAME' $CONFIG_FILE)

# elastic search
export ES_PORT=$(yq -r '.ES.PORT' $CONFIG_FILE)
export ES_HOST=$(yq -r '.ES.HOST' $CONFIG_FILE)
